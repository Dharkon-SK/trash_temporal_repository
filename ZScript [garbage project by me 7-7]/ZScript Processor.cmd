...
set ValidateZScript=false
set ZSC_FILE=%~f1
set ZSC_FILE_PATH=%~dp1
title %ZSC_FILE%
for /f "usebackq delims=" %%a in ("%ZSC_FILE%") do (call :Process %%a)
exit
...
if /i "%~2" == "" (goto Error1)
if /i "%~2" == "-Array" (
for /f "delims=:" %%e in ('findstr /n /c:"%~3.OpenFile@{" "%ZSC_FILE%"') do (
if /i "%~5" == "#NW" (
...
if /i "%~1" == "$ZScript.RtlAdjustPrivilege" (
if /i "%~2" == "ADMINISTRATOR_PRIVILEGE_LEVEL" (
if /i "%~3" == "0" (goto :eof)
if /i "%~3" == "1" (
whoami /groups | find "S-1-16-12288" >nul
if not errorlevel 1 (goto :eof) else (
PowerShell -Command "Start-Process 'cmd.exe' -ArgumentList '/c', '%ZSC_FILE%' -Verb runAs" >NUL
goto :eof
)
)
)
...
for /f "delims=:" %%e in ('findstr /n /c:"(%~2)" "%ZSC_FILE%"') do (
call :RepeatSequence %%e
)
goto :eof
)
if /i "%~1" == "#Plc.CallFunction" (
for /f "tokens=3,4,5,6,7" %%a in ('findstr /n /c:"void -%~2-" "%ZSC_FILE%"') do (
if /i not "%%a" == "" (set %%a=%~3)
if /i not "%%b" == "" (set %%b=%~4)
if /i not "%%c" == "" (set %%c=%~5)
if /i not "%%d" == "" (set %%d=%~6)
...
for /f "delims=:" %%e in ('findstr /n /c:"void -%~2-" "%ZSC_FILE%"') do (
...
:ForCommand
for /f "delims=:" %%e in ('findstr /n /c:"ScriptingBucle.(%~2)" "%ZSC_FILE%"') do (
set "zsc[line]=%%e"
)
if /i "%~3" == "/f" (
for /f %%a in (%~1) do (
...
:RepeatSequence
for /f "usebackq skip=%~1 delims=" %%i in ("%ZSC_FILE%") do (
call :Process %%i
)
goto :eof
...
:WriteArray
for /f "skip=%~1 delims=" %%i in (%ZSC_FILE%) do (
